...
**Now:** Phase 0 **complete** (breadboard proven, audio verified clean, power measured — see Decisions log + Phase 0 checklist). **Phase 2 (KiCad) in progress.** Block 1 is **reviewed and being drawn** in KiCad on the **workstation** (tailnet 100.88.81.67): [`block-1-mcu-power-usb.md`](docs/schematic/block-1-mcu-power-usb.md) carries the full net list with KiCad symbols, footprints and MPNs, pin numbers verified against the `RF_Module:ESP32-S3-WROOM-1` symbol. KiCad project lives at `kicad/room-node/`, six hierarchical sheets, global labels + power symbols (no sheet pins). **Rev A is hand-built** — see Decisions log. **Next up: Block 2 (audio, MAX98357A)** — where the Phase 0 lessons (6 dB GAIN strap, decoupling, star ground, SD mute) get baked in.
...
- 2026-07-28: **Rev A is hand-built, not JLCPCB-assembled.** Cost is close (≈$60–100 hand vs ≈$80–150 assembled once spares and MOQs are counted) and PCBA's ~1 week lead time is tolerable — neither is the reason. The reason is that rev A is a bring-up board for our own house whose job is to be wrong in informative ways: bodging a wire or swapping a value on the bench beats assembly polish, and hand-building removes any dependency on JLCPCB part stock. It also teaches the DFM lessons the Phase 4 pilot needs. **Constrains the design:** 4-layer, ENIG finish, stencil ordered with the boards, single-sided placement, ≥0.5 mm passive spacing, 0603 minimum, through-hole audio bulk cap, tented thermal vias, windowpaned (50–70%) stencil apertures on exposed pads. **Known risk:** the WROOM-1 ground pad and MAX98357A thermal pad can't be inspected once placed — marginal joints there mimic design bugs (resets under WiFi load, amp thermal shutdown). Suspect them first during bring-up. PCBA returns at the Phase 4 pilot.
- 2026-07-28: **MAX98357A kept despite the TQFN-16 exposed pad.** Considered PCM5102A (TSSOP) + PAM8302A (SOP-8) to get two easy packages instead of one hard one, but that is a different audio chain than Phase 0 validated, costs board area, and discards the tuned GAIN/SD behaviour. A stencil solves the actual problem.
